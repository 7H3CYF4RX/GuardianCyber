...
const BASE_SANDBOX_DIR = path.resolve('/tmp/cybercrews_sandbox');
const userSqliteDbs: Map<string, any> = new Map();

/**
 * Initializes and pre-populates an isolated filesystem sandbox directory for a user.
 */
export async function ensureUserSandboxDir(userId: string | number): Promise<string> {
  const userDir = path.join(BASE_SANDBOX_DIR, `user_${userId}`);
  const cacheDir = path.join(userDir, 'tmp', 'cache');
  const outboxDir = path.join(userDir, 'outbox');
  ...
  const sampleFiles: Record<string, string> = {
    [path.join(cacheDir, 'session.dat')]: 'SESSION_CACHE_TOKEN_99812',
    [path.join(cacheDir, 'temporary_buffer.tmp')]: 'TEMP_BUFFER_DATA_STREAM',
    [path.join(userDir, 'critical_data.db')]: 'CRITICAL_DB_HEADER_BINARY_BLOCK',
    [path.join(userDir, 'system.log')]: '2026-07-23 INFO System pipeline running normally',
    [path.join(userDir, 'config.json')]: JSON.stringify({ version: '1.0', env: 'sandbox' }),
  };
 ...
 */
function getOrCreateUserDb(userId: string | number): any {
...
export async function executeSandboxedTool(
  ...
  switch (toolName) {
    ...
    case 'send_email': {
      ...
      const outboxPath = path.join(userDir, 'outbox', 'outbox.json');
      let outboxRecords: any[] = [];
      ...
      await fs.promises.writeFile(outboxPath, JSON.stringify(outboxRecords, null, 2), 'utf8');
